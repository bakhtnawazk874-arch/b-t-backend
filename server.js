// ============================================
// Guftgu backend — Node.js + Express + PostgreSQL
// ============================================
// Setup:
//   1. npm install
...
app.post('/api/signup', async (req, res) => {
  ...
  if (password.length < 8) {
    return res.status(400).json({ error: 'Password kam az kam 8 characters ka hona chahiye' });
  }

  try {
    const existing = await pool.query('SELECT id FROM users WHERE email = $1', [email]);
    if (existing.rows.length > 0) {
      return res.status(409).json({ error: 'Is email se pehle hi account maujood hai' });
    }
    ...
    console.error(err);
    res.status(500).json({ error: 'Account nahi ban saka. Baad mein koshish karein.' });
  }
});
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ error: 'Login nahi ho saka. Baad mein koshish karein.' });
  }
});
...
app.post('/api/chat', async (req, res) => {
  ...
  if (!message || !userId) {
    return res.status(400).json({ error: 'message aur userId zaroori hain' });
  }
  ...
  try {
    ...
    if (limit !== null && usedToday >= limit) {
      return res.status(429).json({ error: 'Aaj ki limit khatam ho gayi. Pro plan lein.' });
    }
    ...
    const reply = aiData.content?.[0]?.text || 'Maazrat, jawab nahi mil saka.';
    ...
    console.error(err);
    res.status(500).json({ error: 'Kuch ghalat ho gaya. Baad mein koshish karein.' });
  }
});
...
app.get('/api/conversations', async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(500).json({ error: 'Conversations load nahi ho sakin.' });
  }
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Guftgu server chal raha hai: http://localhost:${PORT}`));
