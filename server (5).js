...
const WHATSAPP_TOKEN = "";
const WHATSAPP_PHONE_NUMBER_ID = "1329865660207570";
const WHATSAPP_VERIFY_TOKEN = "btai_secret_123";

// Shared helper — sends a message to Groq and returns the AI's reply text.
// Used by both the website chat and the WhatsApp bot.
async function askGroq(message) {
  ...
  return aiData.choices?.[0]?.message?.content || 'Sorry, no reply was received.';
}
...
// Meta sends a POST here every time someone messages your test number.
// We reply using the same Groq AI as the website chat.
// ---------------------------------------------
app.post('/webhook', async (req, res) => {
  // Respond immediately — Meta requires a fast 200 OK, we handle the
  // actual reply asynchronously afterwards.
  res.sendStatus(200);
  ...
  try {
    ...
    if (!message || message.type !== 'text') return; // ignore non-text/status updates
